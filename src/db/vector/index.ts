...
export async function insertVectorColumn(
  pool: pg.Pool,
  ...
  if (
    res &&
    res.rows &&
    !res.rows.find(row => row.column_name == embeddingsColumnName)
  ) {
    const dimensionality = (await embeddings.embedQuery('hello')).length
    logger.info({
      msg: 'Could not find embedding column in the vector table. Adding it...',
      dimensionality
    })
    await client.query(
      `ALTER TABLE document_chunks ADD COLUMN ${embeddingsColumnName} vector(${dimensionality})`
    )
    if (dimensionality <= MAX_DIM_FOR_INDEX) {
      // hnsw indexes are only supported when dimension <= MAX_DIM_FOR_INDEX
      await client.query(
        `CREATE INDEX ON document_chunks USING hnsw (${embeddingsColumnName} vector_cosine_ops)`
      )
    } else {
      logger.warn({
