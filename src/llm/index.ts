...
function reRank(vectorResults:DocumentChunkResult[], keywordResults:DocumentChunkResult[], searchReRankBalance=3.1) {
  const rankedResults:DocumentChunkResult[] = [...vectorResults, ...keywordResults.map(obj => ({...obj, score: obj.score * searchReRankBalance}))]
...
export async function hybridRetrieve(args: RagArgs, conf:SearchConf) {
  ...
  const keywordResults = await db.searchByKeyword(conf.dbPool, keywords, { limit: limit })
...
export async function rag(args: RagArgs, conf:SearchConf) {
  ...
  const compiledRagPrompt = await ragPrompt.format({
    query: args.prompt,
    documents: searchResults.slice(0,3).map(r => r.content).join('/n/n/n')
  });
